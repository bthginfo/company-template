...
} from './modular-medical-blueprints.js';
import {
  str,
...
import { mapModularDoctor, mapModularItemToService } from './modular-catalog-mappers.js';
  ...
  if (sc) {
    sc.data = {
      ...
      items: (content.services ?? []).map((s) => ({
        ...
        tags: '',
        button: { label: 'Termin anfragen', linkType: 'internal', internalPage: '/kontakt', externalUrl: '' },
