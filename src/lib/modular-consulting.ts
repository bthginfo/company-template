...
function emptySections(style: TemplateStyle, page: ConsultingModularPageKey): ModularSectionV1[] {
  return consultingModularBlueprint(style, page).map((type, i) => ({
    id: `${page}-${type}-${i}`,
    type,
    isVisible: true,
    data: {},
...
function mergeConsultingGalleryExtras(content: SiteContent, sections: ModularSectionV1[]): SiteContent {
  let next = content;
...
function mergeConsultingHomeSupplements(content: SiteContent, sections: ModularSectionV1[]): SiteContent {
...
function mergeConsultingServicesIntoLegacy(content: SiteContent, sections: ModularSectionV1[], _style: TemplateStyle): SiteContent {
  let next: SiteContent = { ...content };
...
function importConsultingHome(content: SiteContent, sections: ModularSectionV1[], style: TemplateStyle): void {
  ...
  if (sc) {
    sc.data = {
      ...
      items: (content.services ?? []).map((s) => ({
        ...
        tags: '',
        button: { label: str(s.learnMoreLabel) || 'Mehr', linkType: 'internal', internalPage: str(s.learnMoreHref) || '/leistungen', externalUrl: '' },
  ...
  if (pp) {
    pp.data = {
      eyebrow: 'Pakete',
      headline: 'Klar strukturiert.',
      description: '',
      items: (content.packages ?? []).map((p) => ({
        title: str(p.name),
        badge: p.highlight ? 'Empfohlen' : '',
        price: str(p.price),
...
function importConsultingServices(content: SiteContent, sections: ModularSectionV1[], style: TemplateStyle): void {
  const by = (t: string) => sections.find((s) => s.type === t);
  const nb = by('noticeBanner');
...
export function importConsultingModularFromLegacy(content: SiteContent, style: TemplateStyle): ModularPagesV1 {
  const home = emptySections(style, 'home');
  importConsultingHome(content, [...home], style);
  const services = emptySections(style, 'services');
  importConsultingServices(content, [...services], style);
  ...
  return {
    combo: { template: 'consulting', style },
    home: { sections: home },
...
export function applyConsultingModularToLegacy(content: SiteContent): SiteContent {
  const m = content.modularPagesV1;
  if (!m?.combo || m.combo.template !== 'consulting') return content;
  const style = m.combo.style;
  let next: SiteContent = { ...content };
  if (m.home?.sections?.length) next = mergeConsultingHomeIntoLegacy(next, m.home.sections, style);
  if (m.services?.sections?.length) next = mergeConsultingServicesIntoLegacy(next, m.services.sections, style);
  if (m.gallery?.sections?.length) {
    next = mergeGalleryIntoLegacy(next, m.gallery.sections);
    next = mergeConsultingGalleryExtras(next, m.gallery.sections);
  }
